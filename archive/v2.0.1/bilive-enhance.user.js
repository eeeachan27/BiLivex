...
(function () {
  ...
  async function sendSpecialEmoji(hash, requestId) {
    ...
    return { status: 'queued', message: '已通过原生表情面板发送' };
  }
  ...
  function sendPlusOne(payload) {
    ...
    return enqueuePlusSend(async () => {
      if (normalized.type === 'emoji-special' && normalized.hash) {
        ...
        showToast(result.message || (result.status === 'queued' ? '已通过原生表情面板发送' : '发送失败'));
      ...
      try {
        ...
        cBtn.addEventListener('click', (e) => {
          ...
          runPlusButtonAction(cBtn, () => sendPlusOne(payload), (result) => {
            if (result.status === 'sent' || result.status === 'queued') showFloatingPlusFeedback(clone);
          });
        });
  ...
  function getViewportWidth() {
    try {
      if (panelWindow.visualViewport && panelWindow.visualViewport.width > 0) {
        return panelWindow.visualViewport.width;
      }
    } catch (e) {}
    return panelWindow.innerWidth || panelDocument.documentElement.clientWidth;
  }

  function getViewportHeight() {
    try {
      if (panelWindow.visualViewport && panelWindow.visualViewport.height > 0) {
        return panelWindow.visualViewport.height;
      }
    } catch (e) {}
    return panelWindow.innerHeight || panelDocument.documentElement.clientHeight;
  }

  // 使用样式目标值进行钳制，避免 CSS 过渡中的插值位置覆盖吸附目标。
  function clampPanelToViewport(panel) {
    try {
      ...
      const left = Math.max(
        PANEL_EDGE_MARGIN,
        Math.min(getViewportWidth() - rect.width - PANEL_EDGE_MARGIN, Number.isNaN(curLeft) ? rect.left : curLeft)
      );
      const top = Math.max(
        PANEL_EDGE_MARGIN,
        Math.min(getViewportHeight() - rect.height - PANEL_EDGE_MARGIN, Number.isNaN(curTop) ? rect.top : curTop)
      );
      panel.style.left = left + 'px';
      panel.style.top = top + 'px';
      panel.style.right = 'auto';
  ...
  function computeMenuPosFromBall(panel) {
    const ballLeft = parseFloat(panel.style.left) || panel.getBoundingClientRect().left;
    const ballTop = parseFloat(panel.style.top) || panel.getBoundingClientRect().top;
    const viewportWidth = getViewportWidth();
    const ballCenterX = ballLeft + COLLAPSED_BTN_SIZE / 2;
    const dir = ballCenterX < viewportWidth / 2 ? 'right' : 'left';
    const targetLeft = dir === 'right'
      ? ballLeft + COLLAPSED_BTN_SIZE + PANEL_EDGE_MARGIN
      : ballLeft - MENU_WIDTH - PANEL_EDGE_MARGIN;
    return {
      dir,
      left: Math.max(PANEL_EDGE_MARGIN, Math.min(viewportWidth - MENU_WIDTH - PANEL_EDGE_MARGIN, targetLeft)),
      top: ballTop,
    };
  }

  function computeBallPosFromMenu(panel) {
    const menuLeft = parseFloat(panel.style.left) || panel.getBoundingClientRect().left;
    const menuTop = parseFloat(panel.style.top) || panel.getBoundingClientRect().top;
    const dir = panel.dataset.bilivexExpandDir || 'left';
    const ballLeft = dir === 'right'
      ? menuLeft - COLLAPSED_BTN_SIZE - PANEL_EDGE_MARGIN
      : menuLeft + MENU_WIDTH + PANEL_EDGE_MARGIN;
    return {
      left: Math.max(PANEL_EDGE_MARGIN, Math.min(getViewportWidth() - COLLAPSED_BTN_SIZE - PANEL_EDGE_MARGIN, ballLeft)),
      top: Math.max(PANEL_EDGE_MARGIN, Math.min(getViewportHeight() - COLLAPSED_BTN_SIZE - PANEL_EDGE_MARGIN, menuTop)),
    };
  }

  function getBaseTransition(panel) {
    return (panel && panel.dataset && panel.dataset.bilivexBaseTransition) || '';
  }

  function isAnimTransition(panel) {
    const transition = (panel && panel.style && panel.style.transition) || '';
    return transition.indexOf('300ms') >= 0;
  }
